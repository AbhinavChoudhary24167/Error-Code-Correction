`timescale 1ns/1ps
module tb_safeforge_robust_8_4_v1_safe_safe;
  ...
  integer d;
  safeforge_robust_8_4_v1_safe_encoder u_encoder(.data(data), .codeword(codeword));
  safeforge_robust_8_4_v1_safe_safe_decoder u_decoder(
  ...
  initial begin
    ...
    for (d = 0; d < 16; d = d + 1) begin
      ...
      if (due || data_out !== data) $fatal(1, "policy mismatch for sbu-1 data=%0d", d);
      ...
      if (due || data_out !== data) $fatal(1, "policy mismatch for nonadj-dbu-0-3 data=%0d", d);
      ...
      if (due || data_out !== data) $fatal(1, "policy mismatch for sbu-5 data=%0d", d);
      ...
      if (!due) $fatal(1, "policy mismatch for adj-dbu-4-5 data=%0d", d);
      ...
      if (!due) $fatal(1, "policy mismatch for nonadj-dbu-2-7 data=%0d", d);
    ...
    $display("PASS tb_safeforge_robust_8_4_v1_safe_safe");
    $finish;
  end
endmodule
