# Additive diagnostic: expose one SRAM output net as a temporary top-level
# input in the in-memory ODB so OpenSTA seeds its measured activity as a root.
set out /repo/campaigns/iscas_sustainability_extension/green_v3_3_activity_complete_e5/diagnostics/bterm_activity_sensitivity
file mkdir $out
...
set macro [$block findInst u_data]
set iterm [$macro findITerm {dout[45]}]
set net [$iterm getNet]
puts "GREEN_V33_BTERM_ADAPTER source=[$iterm getName] net=[$net getName]"
$iterm disconnect
set bterm [odb::dbBTerm_create $net green_v33_data_dout_45]
$bterm setSigType SIGNAL
$bterm setIoType INPUT

read_sdc $root/6_final.sdc
read_spef $root/6_final.spef
set_power_activity -input_ports {green_v33_data_dout_45} -density 0.000001 -duty 0.1
...
foreach pin {green_v33_data_dout_45 _393_/A _393_/Y} {
  puts "GREEN_V33_BTERM_LOW pin=$pin value=[get_property -object_type pin $pin activity]"
}

set_power_activity -input_ports {green_v33_data_dout_45} -density 0.1 -duty 0.9
...
foreach pin {green_v33_data_dout_45 _393_/A _393_/Y} {
  puts "GREEN_V33_BTERM_HIGH pin=$pin value=[get_property -object_type pin $pin activity]"
}
