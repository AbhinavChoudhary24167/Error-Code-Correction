...
module tb;
  ...
  initial begin
    failures = 0;
    $readmemh("/mnt/c/Users/Abhinav/OneDrive/Desktop/ECC/Error-Code-Correction/campaigns/iscas_sustainability_extension/green_v3_3_activity_complete_e5/fresh_runs/hsiao_secded_clk10p0ns_seed11/attempt01/workloads/payloads.hex", payloads);
    $readmemh("/mnt/c/Users/Abhinav/OneDrive/Desktop/ECC/Error-Code-Correction/campaigns/iscas_sustainability_extension/green_v3_3_activity_complete_e5/fresh_runs/hsiao_secded_clk10p0ns_seed11/attempt01/workloads/codewords_read_single_bit_error_correct.hex", codewords);
    ...
    rstb = 1'b1;
    for (i = 0; i < 16; i = i + 1) apply_operation(i, 1);
    $dumpfile("/var/lib/green-ecc-v33-activity-complete-e5/fresh_runs/hsiao_secded_clk10p0ns_seed11/attempt01/activity/READ_SINGLE_BIT_ERROR_CORRECT.vcd");
    $dumpvars(1, dut);
    $display("MEASUREMENT_BEGIN_NS=%.3f", $realtime);
    for (i = 0; i < 256; i = i + 1) apply_operation(i, 1);
    #1;
    $display("MEASUREMENT_END_NS=%.3f", $realtime);
    $dumpoff;
    if (failures != 0) $fatal(1, "FUNCTIONAL_FAILURE_COUNT=%0d", failures);
    $display("SIMULATION_PASS operation=READ_SINGLE_BIT_ERROR_CORRECT warmup=16 operations=256");
    $finish;
  end
endmodule
