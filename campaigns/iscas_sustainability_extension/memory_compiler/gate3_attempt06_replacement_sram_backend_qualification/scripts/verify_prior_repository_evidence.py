#!/usr/bin/env python3
"""Freshly rehash repository-resident evidence from Attempts01--05."""
...
campaigns = [
    verify("Attempt01/Attempt02 repository evidence", MEMORY / "GATE2_GATE3_EVIDENCE_MANIFEST.json", MEMORY, "files"),
    verify(
...
payload = {
    "schema_version": 1,
    "algorithm": "sha256",
    "scope": "repository-resident files listed by preserved Attempts01--05 manifests",
    "status": "PASS" if all(row["status"] == "PASS" for row in campaigns) else "FAIL",
    "campaigns": campaigns,
    "external_attempt02_attempt03": {
        "status": "NOT_REVERIFIED",
        "reason": "Ubuntu-24.04 VHDX remains inaccessible with ERROR_SHARING_VIOLATION; no historical external result was promoted to current PASS.",
    },
}
target = ROOT / "raw" / "integrity" / "prior_campaign_repository_verification.json"
target.parent.mkdir(parents=True, exist_ok=True)
target.write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")
print(json.dumps({"status": payload["status"], "campaigns": [{"label": r["label"], "status": r["status"], "files": r["file_count"]} for r in campaigns]}))
