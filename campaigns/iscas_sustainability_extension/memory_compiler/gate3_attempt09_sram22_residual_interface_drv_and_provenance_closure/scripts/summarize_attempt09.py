#!/usr/bin/env python3
"""Extract auditable Attempt09 five-seed final-route metrics."""
...
DESIGNS = {"U0": "attempt09_u0", "E0": "attempt09_e0"}
CLASS_C = {"U0": 64, "E0": 72}
...
def collect(design: str, flow: str, seed: int) -> dict:
    ...
    genuine_external = slew - CLASS_C[design]
    basic = not missing and drc == setup == hold == cap == antenna == 0
    external_timing_drv_clean = basic and genuine_external == 0
    return {
        "design": design,
        "seed": seed,
        ...
        "max_slew_violations_reported": slew,
        "class_c_sram_output_rows": CLASS_C[design],
        "genuine_external_slew_violations": genuine_external,
        ...
        "provenance_limited_exception_eligible": basic and genuine_external == 0,
...
def main() -> None:
    ...
    payload = {
        ...
        "same_interface_repair_and_common_hold_policy_all_e0_seeds": True,
        "u0_attempt08_methodology_unchanged": True,
        ...
        "classification": "RESIDUAL_SRAM_INPUT_DRV_FAIL",
        "provenance_limited_exception_used": False,
        "classification_note": "Each design retains one explicit 0.351 ns 256x64 SRAM rstb violation; Class-C output rows are tracked separately and never waived.",
        "seeds": seeds,
    }
    rendered = json.dumps(payload, indent=2) + "\n"
    (ROOT / "raw" / "per_seed_metrics.json").write_text(rendered, encoding="utf-8")
    (ROOT / "MULTISEED_EXTERNAL_CLOSURE.json").write_text(rendered, encoding="utf-8")
