#!/usr/bin/env python3
"""Summarize final regression logs against the frozen Attempt07 failure set."""
...
def pytest_result(name: str, expected_frozen: bool) -> dict:
    ...
    return {
        ...
        "passed": passed,
        "failed": failed,
        "failure_nodeids": sorted(failures),
        "log": f"raw/regression/{name}",
    }
...
make = {
    "status": "PASS" if "make: ***" not in make_text and "error:" not in make_text.lower() else "FAIL",
    "exit_code": 0 if "make: ***" not in make_text and "error:" not in make_text.lower() else 1,
    "log": "raw/regression/make.log",
    "toolchain": "Windows GNU make / MSYS-compatible g++",
}
