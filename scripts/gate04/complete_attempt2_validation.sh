...
test ! -e "$run_root/amendment02-validation-metadata.json"
...
bash "$snapshot/scripts/gate04/mapped_equivalence.sh" \
  "$run_root" "$snapshot" "$top" \
  "scripts/gate03r/rtl/secded_characterization_tops.sv,scripts/gate04/rtl/gate04_boundaries.sv" \
  mapped "$run_root/mapped-equivalence.log"
bash "$snapshot/scripts/gate04/mapped_equivalence.sh" \
  "$run_root" "$snapshot" "$top" \
  "scripts/gate03r/rtl/secded_characterization_tops.sv,scripts/gate04/rtl/gate04_boundaries.sv" \
  postroute "$run_root/postroute-equivalence.log"
...
python3 -c 'import datetime,json,pathlib; pathlib.Path("'"$run_root"'/amendment02-validation-metadata.json").write_text(json.dumps({"schema_version":1,"status":"PASS","amendment":"02","completed_at_utc":datetime.datetime.now(datetime.timezone.utc).replace(microsecond=0).isoformat().replace("+00:00","Z"),"original_runner_exit_status":93,"physical_flow_reused_without_rerun":True,"mapped_equivalence":"PASS","postroute_equivalence":"PASS","activity_power":"PASS","run_validation":"PASS"},indent=2,sort_keys=True)+"\n",encoding="utf-8")'
(
  cd "$run_root"
  find . -type f ! -name raw-artifacts-amendment02.sha256 -print0 | sort -z | xargs -0 sha256sum > raw-artifacts-amendment02.sha256
)
echo "GATE04_ATTEMPT2_ADJUDICATION_PASS"
