#!/usr/bin/env python3
"""Gate 03E-R adjudication, freeze, inventory, comparison, and validation."""
...
RUN_ROOT_RE = re.compile(r"/(?:gate03e|gate03er)-run-\d{2}")
...
def compare_runs(
    ...
    if first.get("policy_sha256") != expected_policy_hash or second.get("policy_sha256") != expected_policy_hash:
        failures.append("run inventory policy hash does not match frozen policy v2")
    ...
    if additional:
        failures.append(f"{len(additional)} artifacts additional in run 4")
    ...
    for relative in shared:
        ...
        if not raw_equal:
            if canonical_equal:
                classification = "execution runtime/resource usage or serialization/path/timestamp metadata"
                passed = True
            elif suffix in SEMANTIC_SUFFIXES or suffix == ".odb" or _semantic_match(relative, policy):
                classification = "scientific QoR/state"
                passed = False
            elif suffix == ".lyt":
                classification = "scientific QoR/state"
                ...
                classification = "scientific QoR/state or unresolved"
                passed = False
            else:
            ...
            raw_differences.append(
                {
                    "path": relative,
                    "run_3_raw_sha256": left["raw_sha256"],
                    "run_4_raw_sha256": right["raw_sha256"],
                    "run_3_canonical_sha256": left["canonical_sha256"],
                    "run_4_canonical_sha256": right["canonical_sha256"],
                    "classification": classification,
                    "pass": passed,
        ...
        if suffix == ".odb" and not raw_equal:
            failures.append(f"{relative}: stage ODB raw hash mismatch")
        if suffix == ".lyt":
            technology.append(
                {
                    "path": relative,
    ...
    for rule in schema["metrics"]:
        ...
        if not rule["gating"]:
            runtime_observations.append(
                {"path": identity, "run_3": left_value, "run_4": right_value, "classification": rule["classification"]}
            )
            continue
    ...
    for relative in ("results/sky130hd/gcd/base/1_2_yosys.v", "results/sky130hd/gcd/base/6_final.v"):
        if (root_1 / relative).is_file() and (root_2 / relative).is_file():
            ...
            histograms.append({"path": relative, "run_3": left_hist, "run_4": right_hist, "pass": passed})
            if not passed:
    ...
    return {
        ...
        "missing_from_run_4": missing,
        "additional_in_run_4": additional,
...
def freeze_bundle(destination: Path, named_files: list[tuple[str, Path]]) -> dict[str, Any]:
    ...
    manifest = {
        "schema_version": 1,
        "policy_id": "gate03er-reproducibility-v2",
...
def validate_policy(policy_path: Path, catalog_path: Path, schema_path: Path | None = None) -> list[str]:
    ...
    policy = read_json(policy_path)
    catalog = read_json(catalog_path)
    if policy.get("policy_id") != "gate03er-reproducibility-v2":
        errors.append("unexpected policy ID")
    if policy.get("amends", {}).get("sha256") != V1_POLICY_SHA256:
        errors.append("policy v1 amendment anchor changed")
    anti = policy.get("anti_posthoc_rules", {})
    if anti.get("field_specific_exclusions_selected_because_policy_v1_failed") != "prohibited":
    ...
    if schema_path:
        ...
        for row in schema.get("metrics", []):
            ...
            if row.get("classification") not in {"scientific QoR/state", "execution runtime/resource usage"}:
...
def validate_inputs(manifest_path: Path) -> list[str]:
    ...
    for row in manifest.get("gate03er_implementation_files", []):
        path = Path(row["path"])
        if not path.is_file() or sha256_file(path) != row["sha256"]:
            errors.append(f"Gate 03E-R implementation hash mismatch: {path}")
    ...
    try:
        image_id = subprocess.check_output(
            ["docker", "image", "inspect", "--format", "{{.Id}}", f"openroad/orfs@{IMAGE_DIGEST}"], text=True
        ).strip()
        if image_id != IMAGE_DIGEST:
            errors.append(f"OCI image ID mismatch: {image_id}")
        script = """
set -euo pipefail
...
"""
        ...
        observed = [
            {"name": parts[0], "path": parts[1], "sha256": parts[2]}
...
def main() -> int:
    ...
    if args.command == "compare":
        result = compare_runs(args.run_1, args.run_2, args.policy, args.schema)
        write_json(args.output, result)
