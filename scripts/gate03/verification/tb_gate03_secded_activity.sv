...
module gate03_tb_secded_activity #(
  ...
  always #5 clk = ~clk;
  secded_encoder #(.DATA_W(64)) reference_encoder(
    .data_i(data), .codeword_o(clean_codeword), .parity_dbg_o()
  );
  assign decoder_input = clean_codeword ^ testbench_mask;
  gate03_secded_decoder_shell dut(
  ...
  initial begin
    $dumpfile("gate03-secded-activity.vcd");
    $dumpvars(0, dut);
    $dumpoff;
    testbench_mask = '0;
    ...
    @(negedge clk);
    $dumpoff;
    $display("GATE03_ACTIVITY_PASS secded fault_weight=%0d measured_cycles=8192", FAULT_WEIGHT);
    $finish;
  end
endmodule
