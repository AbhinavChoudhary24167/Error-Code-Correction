...
public:
    ...
    void testKnownVectors() {
        ...
        struct Vector { uint64_t data; uint64_t encoded_low; uint16_t encoded_high; };
        std::vector<Vector> vectors = {
            {0x0ULL, 0x0ULL, 0x0},
            {0xFFFFFFFFFFFFFFFFULL, 0xFFFFFFFFFFFFFFFFULL, 0xFF},
            {0x0123456789ABCDEFULL, 0x48D159E23579DEFCLL, 0x80}
        };

        for (const auto& v : vectors) {
            auto cw = hamming.encode(v.data);
            assert(cw.data_low == v.encoded_low && cw.data_high == v.encoded_high && "Encoding mismatch");
